...
@views function TwoPhasesPressures()
    ...
    iterMax = 10000#3e4          # max number of iters
    ...
    ηv_v[radv.<1].= ηinc
    if viscoelastic
        ηc_ve  .= (1.0./(G*dt_phys) .+ 1.0./ηc_v).^-1
        ηv_ve  .= (1.0./(G*dt_phys) .+ 1.0./ηv_v).^-1
    else
        ηc_ve  .= ηc_v
        ηv_ve  .= ηv_v
    end
    ...
    for it = 1:nt
        iter=1; err=2*ε;
        # Previous time step
        τxx0 .= τxx; τyy0 .= τyy; τxy0 .= τxy; τxyv0 .= τxyv; 
        ln1mϕ0   .= ln1mϕ;   Pf0  .= Pf;  Pt0  .= Pt
        @printf("it = %d\n", it) 
        while (err>ε && iter<=iterMax)
            # Assign corrected pressure to trial pressure
            ϕ      .= 1.0 .- exp.(ln1mϕ)
            # BCs
            ...
            Exx1   .=    Exx   .+ τxx0 ./2.0./(G*dt_phys)
            Eyy1   .=    Eyy   .+ τyy0 ./2.0./(G*dt_phys)
            Exyv1  .=    Exyv  .+ τxyv0./2.0./(G*dt_phys)
            Exy1   .= av(Exyv) .+ τxy0 ./2.0./(G*dt_phys)
            ...
            dtϕ    .= 2.1.*ηc_ve./max(nx,ny) 
            # Residuals
            # aϕ     .= ((Pt1.-Pf1)./η_ϕ + ((Pt1.-Pt0).-(Pf1.-Pf0))/dt_phys.*β_ϕ)./(1.0.-ϕ) - ∇Vp
            # Rϕ     .= aϕ .- (ln1mϕ .- ln1mϕ0)./dt_phys
            RPt    .= .-( ∇V  .+ (Pt.-Pf)./η_ϕ./(1.0.-ϕ) .+ 1.0 ./Kd .* (  (Pf.-Pf0)./dt_phys .- α       .* (Pt.-Pt0)./dt_phys ) )
            RPf    .= .-( ∇qD .- (Pt.-Pf)./η_ϕ./(1.0.-ϕ) .- α   ./Kd .* (  (Pt.-Pt0)./dt_phys .- 1.0 ./B .* (Pf.-Pf0)./dt_phys ) )
        ...
        # Plotting
        p1 = heatmap(xc, yc, ϕ_viz' , aspect_ratio=1, xlim=extrema(xv), ylim=extrema(yv), c=:inferno, title="ϕ - visu only")
        ...
        p2 = plot( evo_t./dt_phys, evo_τII, label="ΔTii", xlabel="time", ylabel="Tii, Pt, Pf" )
        p2 = plot!(evo_t./dt_phys, evo_Pt, label="ΔPt")
        p2 = plot!(evo_t./dt_phys, evo_Pf, label="ΔPf")
