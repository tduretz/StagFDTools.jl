...
@views function main(nc, nt, layering, BC_template, D_template, factorization, η1 , η2, n1, n2, G1, G2, C1, C2)
    #--------------------------------------------#   
    ...
    # display(to)
    τxxev = mean(τ.xx[inner_x, inner_y])
    τyyev = mean(τ.yy[inner_x, inner_y])
    τxyev = mean( av2D(τ.xy[inner_x, inner_y]))
...
let
    ...
    for iθ in eachindex(θ)
        ...
        τ_cart_lay[iθ, :] = main( nc, nt, layering, BCs[1], D_BCs[1], :lu, η1, η2, n1, n2, G1, G2, C1, C2)
