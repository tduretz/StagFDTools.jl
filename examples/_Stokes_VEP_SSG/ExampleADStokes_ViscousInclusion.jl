...
@views function main(BC_template, D_template)
    ...
    rad = 0.1 + 1e-13
    @views a.phases.c[inx_c, iny_c][(a.X.c.x.^2 .+ (a.X.c.y').^2) .<= rad^2] .= 2
    @views a.phases.v[inx_v, iny_v][(a.X.v.x.^2 .+ (a.X.v.y').^2) .<= rad^2] .= 2
    FillPhaseRatios!(a)
    ...
    for it=1:nt
        ...
        hm1 = heatmap!(ax1, a.X.v.x, a.X.v.y, a.V.x[inx_Vx, iny_Vx]', colormap=:redsblues)
        ...
        scatter!(ax4, 1:iter, log10.(err.x[1:iter]), markersize=8, label="Vx")
        scatter!(ax4, 1:iter, log10.(err.y[1:iter]), markersize=8, label="Vy")
        scatter!(ax4, 1:iter, log10.(err.p[1:iter]), markersize=8, label="Pt")
