...
@views function main(nc, θgouge)
    ...
    phases.c[inx_c, iny_c][(X.c.x.^2 .+ (X.c.y' .- X.v.y[1]).^2) .<= rad^2] .= 2
    phases.v[inx_v, iny_v][(X.v.x.^2 .+ (X.v.y' .- X.v.y[1]).^2) .<= rad^2] .= 2
    phases.v[:, end-1] .= 3
    phases.c[:, end-1] .= 3
    ...
    for it=1:nt
        ...
        function figure()
            ...
            crange = (-17, -13)
            hm = heatmap!(ax, X.c.x.*sc.L, X.c.y.*sc.L, field, colormap=Makie.Reverse(:bilbao), colorrange=crange)
            contour!(ax, X.c.x.*sc.L, X.c.y.*sc.L,  phases.c[inx_c,iny_c], color=:black)
