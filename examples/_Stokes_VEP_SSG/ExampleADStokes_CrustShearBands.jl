...
@views function main(nc, θgouge)
    ...
    for i in inx_c, j in (nc.y+2-1):-1:2
        ...
        # ∫ (-ρ̄ g) dz (g < 0)
        a.Pt[i,j] = a.Pt[i,j+1] - ρ̄ * materials.g[2] .* Δ.y
    end
    ...
    @views a.phases.c[inx_c, iny_c][(a.X.c.x.^2 .+ (a.X.c.y' .- a.X.v.y[1]).^2) .<= rad^2] .= 2
    @views a.phases.v[inx_v, iny_v][(a.X.v.x.^2 .+ (a.X.v.y' .- a.X.v.y[1]).^2) .<= rad^2] .= 2
    a.phases.v[:, end-1] .= 3
    a.phases.c[:, end-1] .= 3
    FillPhaseRatios!(a)
    ...
    for it=1:nt
        ...
        function figure()
            ...
            crange = (-17, -13)
            hm = heatmap!(ax, a.X.c.x.*sc.L, a.X.c.y.*sc.L, field, colormap=Makie.Reverse(:bilbao), colorrange=crange)
            contour!(ax, a.X.c.x.*sc.L, a.X.c.y.*sc.L,  a.phases.c[inx_c,iny_c], color=:black)
            ...
            scatter!(ax, 1:iter, log10.(err.x[1:iter]./err.x[1]), color=:blue )
            scatter!(ax, 1:iter, log10.(err.y[1:iter]./err.y[1]), color=:green )
            scatter!(ax, 1:iter, log10.(err.p[1:iter]./err.p[1]), color=:red )
            ylims!(ax, -15, 1)
            ...
            for i in eachindex(P_ax), j in eachindex(τ_ax)
                m = materials
                mp = m.plasticity
                if mp isa DruckerPrager1 || mp isa DruckerPrager
                    yieldf = DruckerPrager1()
                    p = (mp.C[1], mp.cosϕ[1], mp.sinϕ[1], mp.cosψ[1], mp.sinψ[1], 0*mp.ηvp[1])
                elseif mp isa Golchin2021
                    yieldf = Golchin2021()
                    p = (M=mp.M[1], N=mp.N[1], Pt=-mp.σT[1], Pc=mp.Pc[1], α=mp.a[1], β=mp.b[1], γ=mp.c[1], ηvp=mp.ηvp[1])
                elseif mp isa DruckerHyperbolic
