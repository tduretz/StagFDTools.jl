...
@views function main(n)
    ...
    for i=1:size(a.Pt,1), j=1:size(a.Pt,2)
        sol = Stokes2D_Schmid2003( [a.X.c_e.x[i], a.X.c_e.y[j]]; params )
        Pt_ana[i,j] = sol.p
    end

    # Get Vx analytics
    for i=1:size(BC.Vx,1), j=2:size(BC.Vx,2)-1
        sol = Stokes2D_Schmid2003( [a.X.v_e.x[i], a.X.c_e.y[j-1]]; params )
        BC.Vx[i,j]   =  sol.V[1]
        a.V.x[i,j]   = sol.V[1]
        V_ana.x[i,j] = sol.V[1]
    end
    ...
    for i=2:size(BC.Vy,1)-1, j=1:size(BC.Vy,2)
        sol = Stokes2D_Schmid2003( [a.X.c_e.x[i-1], a.X.v_e.y[j]]; params )
        BC.Vy[i,j]   = sol.V[2]
        a.V.y[i,j]   = sol.V[2]
        V_ana.y[i,j] = sol.V[2]
    end
