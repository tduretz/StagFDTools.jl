...
@views function main(D_BC, nc, nt, n_nt; 
    homo=false, niter=20, Φini=5e-2, ηvp=0.0, r_fact=1.0, ε̇_fact=1.0, visualization=true, n_CK=0.0)
    ...
    materials.η0    .= [ 1e42,   1e42 ]/sc.σ/sc.t 
    materials.ξ0    .= [ 2e42,   2e42 ]/sc.σ/sc.t
    materials.G     .= [ 3e10,   3e9  ]./sc.σ 
    ...
    x = (min=-0*L.x/2, max=L.x/1)
    y = (min=-0*L.y/2, max=L.y/1)
    ...
    #     # # end
        ...
        function figure()
            ...
            # field = log10.(abs.(R.x[inx_v,iny_v] * (sc.σ/sc.L)))
            # hm    = heatmap!(ax, X.v.x, X.c.y, field, colormap=:vik)
...
function Run()
    ...
    # main(D_BC, nc, nt, n_nt, homo=true, niter=2, visualization=false)
    ...
    # with or without eta_vp
    n_nx = 2
    n_nt = 1
    nc   = (x=n_nx*50, y=n_nx*50)
    nt   = 1*n_nt
    D_BC = @SMatrix([-1 0; 0 1] )
    main(D_BC, nc, nt, n_nt; ηvp=1*1e19, homo=false, n_CK=0.0, r_fact=1, ε̇_fact=2.5); #1e20
