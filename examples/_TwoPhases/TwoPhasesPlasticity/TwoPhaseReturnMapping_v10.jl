...
function ΔPΦ(λ̇, Pt0, Pf0, Φ0, p)
    ...
    for iter=1:10
        ...
        J  = FD.jacobian(x -> ΔPΦ_residual(x, λ̇, Pt0, Pf0, Φ0, p), x)
        r  = ΔPΦ_residual(x, λ̇, Pt0, Pf0, Φ0, p)
        x -= J\r
        nr = mynorm(r)
        # @show J
        if iter==1 && nr>1e-17
...
function StressVector_trial(ϵ̇::SVector{N,T}, divVs, divqD, τ0, Pt0, Pf0, Φ0, p) where {N,T}
    ...
    if f>-tol
        ...
        for iter=1:10
            J = FD.jacobian(x -> residual_two_phase_trial(x, ε̇II_eff, Pt_trial, Pf_trial, Φ_trial, divVs, divqD, Pt0, Pf0, Φ0, p), x)
            r = residual_two_phase_trial(x, ε̇II_eff, Pt_trial, Pf_trial, Φ_trial, divVs, divqD, Pt0, Pf0, Φ0, p)
            # display(J.derivs[1])
            x -= J\r
            if iter==1 
                r0 = norm(J)
...
function two_phase_return_mapping()
    ...
    # return @SVector[τ[1], τ[2], τ[3], Pt, Pf, λ̇, Φ1, r]
        ...
        # Stress evaluation
        σ  = StressVector_trial(ϵ̇, divVs, divqD, τ0, Pt0, Pf0, Φ0, params)
        τ, Pt, Pf  = SA[σ[1], σ[2], σ[3]], σ[4], σ[5]
        λ̇, Φ, r = σ[6], σ[7], σ[8]
        # @show τ, Pt, Pf, Φ

        # Consistent tangent by deriving stress w.r.t. input vector 
        pseudo_J = FD.jacobian(ϵ̇->StressVector_trial(ϵ̇,  divVs, divqD, τ0, Pt0,  Pf0, Φ0, params), ϵ̇) # pseudo_J because the 3 last rows are not really part of ∂τ/∂ϵ̇
        J = SMatrix{5,5}(pseudo_J[i,j] for i in 1:5, j in 1:5) # get rid of last 3 rows
        
        # # im not sure what the left hand sides are here
