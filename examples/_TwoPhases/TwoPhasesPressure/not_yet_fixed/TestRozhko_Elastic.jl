...
@views function main(nc, Ωl, Ωη)
    ...
    Ks[inx_c, iny_c][(xc.^2 .+ (yc').^2) .> r_out^2] .= Kso
    Kf[inx_c, iny_c][(xc.^2 .+ (yc').^2) .> r_out^2] .= Kfo
    KΦ[inx_c, iny_c][(xc.^2 .+ (yc').^2) .> r_out^2] .= KΦo
    ...
    Kd = Ks # (1-ϕ0) .* ( 1 ./ KΦ + 1 ./ Ks).^-1
    α  = 0*( 1 .- Kd ./ Ks )
    B  = 0* @. (Kd.^-1 - Ks.^-1) ./ (Kd.^-1 - Ks.^-1 + ϕ.*(Kf.^-1 .- Ks.^-1))
