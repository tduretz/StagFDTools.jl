...
function SMomentum_x_Generic(Vx_loc, Vy_loc, Pt, ΔP, τ0, G_loc, 𝐷, materials, type, bcv, Δ)
    ...
    # More averages
    Pt_v = SVector(av(Pt))
    τ0xx_c = SVector{2}(τ0.xx[i, 2] for i in 1:2)
    τ0yy_c = SVector{2}(τ0.yy[i, 2] for i in 1:2)
    τ0xy_c = SVector(av(τ0.xy))
    τ0xx_v = SVector(av(τ0.xx))
    τ0yy_v = SVector(av(τ0.yy))
    τ0xy_v = SVector{2}(τ0.xy[2, i] for i in 1:2)

    # Velocity gradient - centroids
    ∂Vx∂x = ∂x(Vx) .* invΔx
    Dxx_c = SVector{2}(∂Vx∂x[i, 2] for i in 1:2)
    ∂V̄x∂y = (∂y(V̄x) * invΔy)
    Dxy_c = SVector{2}(∂V̄x∂y[i] for i in 1:2)
    ∂Vy∂y = ∂y(Vy) * invΔy
    Dyy_c = SVector{2}(∂Vy∂y[i, 2] for i in 2:3)
    ∂V̄y∂x = ∂x(V̄y) * invΔx
    Dyx_c = SVector{2}(∂V̄y∂x[i, 2] for i in 1:2)

    # Velocity gradient - vertices
    ∂V̄x∂x = ∂x(V̄x) * invΔx
    Dxx_v = SVector{2}(∂V̄x∂x[i] for i in 1:2)
    ∂Vx∂y = ∂y(Vx) * invΔy
    Dxy_v = SVector{2}(∂Vx∂y[2, i] for i in 1:2)
    ∂V̄y∂y = ∂y(V̄y) * invΔy
    Dyy_v = SVector{2}(∂V̄y∂y[2, i] for i in 1:2)
    ∂Vy∂x = ∂x(Vy) * invΔx
    Dyx_v = SVector{2}(∂Vy∂x[2, i] for i in 2:3)
    ...
    Gc = SVector{2}(G_loc.c[i, 1] for i in 1:2)
    Gv = SVector{2}(G_loc.v[1, i] for i in 1:2)
    ...
    Ptc = SVector{2}(Pt[i, 2] + comp * ΔP[i] for i in 1:2)

    # Stress
    σxx = SVector{2}(
        (𝐷.c[i][1, 1] - 𝐷.c[i][4, 1]) * ϵ̇xx_c[i] + (𝐷.c[i][1, 2] - 𝐷.c[i][4, 2]) * ϵ̇yy_c[i] + (𝐷.c[i][1, 3] - 𝐷.c[i][4, 3]) * ϵ̇xy_c[i] + (𝐷.c[i][1, 4] - (𝐷.c[i][4, 4] - 1)) * Pt[i, 2] - Ptc[i] for i in 1:2
    )
    τxy = SVector{2}(
        𝐷.v[i][3, 1] * ϵ̇xx_v[i] + 𝐷.v[i][3, 2] * ϵ̇yy_v[i] + 𝐷.v[i][3, 3] * ϵ̇xy_v[i] + 𝐷.v[i][3, 4] * Pt_v[i] for i in 1:2
    )
...
function SMomentum_y_Generic(Vx_loc, Vy_loc, Pt, ΔP, τ0, G_loc, ρ_loc, 𝐷, materials, type, bcv, Δ)
    ...
    # # More averages
    Pt_v = SVector(av(Pt))
    τ0xx_c = SVector{2}(τ0.xx[2, i] for i in 1:2)
    τ0yy_c = SVector{2}(τ0.yy[2, i] for i in 1:2)
    τ0xy_c = SVector(av(τ0.xy))
    τ0xx_v = SVector(av(τ0.xx))
    τ0yy_v = SVector(av(τ0.yy))
    τ0xy_v = SVector{2}(τ0.xy[i, 2] for i in 1:2)

    # Velocity gradient - centroids
    ∂Vx∂x = ∂x(Vx) * invΔx
    Dxx_c = SVector{2}(∂Vx∂x[2, i] for i in 2:3)
    ∂V̄x∂y = ∂y(V̄x) * invΔy
    Dxy_c = SVector{2}(∂V̄x∂y[2, i] for i in 1:2)
    ∂Vy∂y = ∂y(Vy) * invΔy
    Dyy_c = SVector{2}(∂Vy∂y[2, i] for i in 1:2)
    ∂V̄y∂x = ∂x(V̄y) * invΔx
    Dyx_c = SVector{2}(∂V̄y∂x[i] for i in 1:2)

    # Velocity gradient - vertices
    ∂V̄x∂x = ∂x(V̄x) * invΔx
    Dxx_v = SVector{2}(∂V̄x∂x[i, 2] for i in 1:2)
    ∂Vx∂y = ∂y(Vx) * invΔy
    Dxy_v = SVector{2}(∂Vx∂y[i, 2] for i in 2:3)
    ∂V̄y∂y = ∂y(V̄y) * invΔy
    Dyy_v = SVector{2}(∂V̄y∂y[i] for i in 1:2)
    ∂Vy∂x = ∂x(Vy) * invΔx
    Dyx_v = SVector{2}(∂Vy∂x[i, 2] for i in 1:2)
    ...
    # Effective visco-elastic strain rate
    Gc = SVector{2}(G_loc.c[1, i] for i in 1:2)
    Gv = SVector{2}(G_loc.v[i, 1] for i in 1:2)
    _2GΔt_c = @. inv(2 * Gc * Δ.t)
    _2GΔt_v = @. inv(2 * Gv * Δ.t)
    ϵ̇xx_c, ϵ̇yy_c, ϵ̇xy_c = effective_strain_rate(ε̇xx_c, ε̇yy_c, ε̇xy_c, τ0xx_c, τ0yy_c, τ0xy_c, _2GΔt_c)
    ϵ̇xx_v, ϵ̇yy_v, ϵ̇xy_v = effective_strain_rate(ε̇xx_v, ε̇yy_v, ε̇xy_v, τ0xx_v, τ0yy_v, τ0xy_v, _2GΔt_v)
    ...
    # Corrected pressure
    comp = materials.compressible
    Ptc = SVector{2}(Pt[2, i] + comp * ΔP[i] for i in 1:2)

    # Stress
    σyy = SVector{2}(
        (𝐷.c[i][2, 1] - 𝐷.c[i][4, 1]) * ϵ̇xx_c[i] + (𝐷.c[i][2, 2] - 𝐷.c[i][4, 2]) * ϵ̇yy_c[i] + (𝐷.c[i][2, 3] - 𝐷.c[i][4, 3]) * ϵ̇xy_c[i] + (𝐷.c[i][2, 4] - (𝐷.c[i][4, 4] - 1.0)) * Pt[2, i] - Ptc[i] for i in 1:2
    )
    τxy = SVector{2}(
        𝐷.v[i][3, 1] * ϵ̇xx_v[i] + 𝐷.v[i][3, 2] * ϵ̇yy_v[i] + 𝐷.v[i][3, 3] * ϵ̇xy_v[i] + 𝐷.v[i][3, 4] * Pt_v[i] for i in 1:2
    )
    ...
    # Gravity
    ρ = SVector{2}(ρ_loc[1, i] for i in 1:2)
...
function SetRHS!(r, R, number, type, nc)
    ...
    Threads.@threads for j in 2:(nc.y + 3 - 1)
        for i in 2:(nc.x + 3 - 1)
            if type.Vx[i, j] == :in
                ind = number.Vx[i, j]
                r[ind] = R.x[i, j]
    ...
    return Threads.@threads for j in 2:(nc.y + 1)
        for i in 2:(nc.x + 1)
            if type.Pt[i, j] == :in
...
function UpdateSolution!(V, Pt, dx, number, type, nc)
    ...
    Threads.@threads for j in 1:size(V.x, 2)
        for i in 1:size(V.x, 1)
            if type.Vx[i, j] == :in
                ind = number.Vx[i, j]
                V.x[i, j] += dx[ind]
            end
    ...
    Threads.@threads for j in 1:size(V.y, 2)
        for i in 1:size(V.y, 1)
            if type.Vy[i, j] == :in
                ind = number.Vy[i, j] + nVx
                V.y[i, j] += dx[ind]
            end
    ...
    # Set S/N periodicity
    return Threads.@threads for i in 2:(nc.x + 3 - 1)
...
@views function TangentOperator!(𝐷, 𝐷_ctl, τ, τ0, ε̇, λ̇, η, G, V, Pt, Pt0, ΔPt, type, BC, materials, phase_ratios, Δ)
    ...
    # Loop over centroids
    Threads.@threads for j in (1 + s):(size(ε̇.xx, 2) - s)
        for i in (1 + s):(size(ε̇.xx, 1) - s)
    ...
    # Loop over vertices
    Threads.@threads for j in (1 + s):(size(ε̇.xy, 2) - s)
        for i in (1 + s):(size(ε̇.xy, 1) - s)
