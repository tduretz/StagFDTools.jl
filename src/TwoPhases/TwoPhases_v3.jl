...
function SMomentum_y_Generic(Vx_loc, Vy_loc, Pt_loc, Pf_loc, ΔP, Pt0, Pf0, Φ0, τ0, 𝐷, phases, materials, type, bcv, Δ)
    ...
    #     Φ         = @. Φ0
    # else
    Φ = SVector{2}(Porosity(Φ0[ii], Ptc[ii], Pfc[ii], Ptc0[ii], Pfc0[ii], KΦ[ii], ηΦ[ii], m[ii], 0.0, 0.0, Δ.t)[1] for ii in eachindex(Φ0))
    # end
...
function Continuity(Vx, Vy, Pt_loc, Pf_loc, old, phase, materials, type, bcv, Δ)
    ...
    if materials.linearizeϕ ||  materials.single_phase
        ...
        Φ = SMatrix{3, 3}(Porosity(Φ0[ii], Pt[ii], Pf[ii], Pt0[ii], Pf0[ii], KΦ[ii], ηΦ[ii], m[ii], 0.0, 0.0, Δt)[1] for ii in eachindex(Φ0))
        dΦdt = SMatrix{3, 3}(Porosity(Φ0[ii], Pt[ii], Pf[ii], Pt0[ii], Pf0[ii], KΦ[ii], ηΦ[ii], m[ii], 0.0, 0.0, Δt)[2] for ii in eachindex(Φ0))
    end
...
function ResidualContinuity2D!(R, V, P, old, phases, materials, number, type, BC, nc, Δ)
    ...
    for j in (1 + shift.y):(nc.y + shift.y), i in (1 + shift.x):(nc.x + shift.x)
        ...
        # bcv_loc    = (x=bcx_loc, y=bcy_loc)
        # type_loc   = (x=typex_loc, y=typey_loc, pt=typept_loc)
        ...
        R.pt[i, j] = Continuity(Vx_loc, Vy_loc, Pt, Pf, old_loc, phase, materials, type_loc, bcv_loc, Δ)
...
function AssembleContinuity2D!(K, V, P, old, phases, materials, num, pattern, type, BC, nc, Δ)
    ...
    for j in (1 + shift.y):(nc.y + shift.y), i in (1 + shift.x):(nc.x + shift.x)
        ...
        ∂Vx, ∂Vy, ∂Pt_loc, ∂Pf_loc = ad_partial_gradients(Continuity, (Vx_loc, Vy_loc, Pt, Pf), old_loc, phase, materials, type_loc, bcv_loc, Δ)
        ∂R∂Vx .= ∂Vx
        ∂R∂Vy .= ∂Vy
        ∂R∂Pt .= ∂Pt_loc
        ∂R∂Pf .= ∂Pf_loc
...
function GlobalResidual!(α, dx, R, V, P, ε̇, τ, ΔP, P0, Φ, Φ0, τ0, λ̇, η, 𝐷, 𝐷_ctl, number, type, BC, materials, phases, nc, Δ)
    ...
    return ResidualFluidContinuity2D!(R, V, P, ΔP, P0, Φ0, phases, materials, number, type, BC, nc, Δ)
end
...
function BackTrackingLineSearch!(rvec, α, dx, R0, R, V, P, ε̇, τ, Vi, Pi, ΔP, P0, Φ, Φ0, τ0, λ̇, η, 𝐷, 𝐷_ctl, number, type, BC, materials, phases, nc, Δ; α_init = 1.0, β = 0.5, c = 1.0e-4)
